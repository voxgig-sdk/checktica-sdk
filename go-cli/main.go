// Command Checktica-cli is a CLI + REPL for the Checktica SDK,
...
const prompt = "checktica"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "detect"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CHECKTICA_APIKEY (api key), CHECKTICA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CHECKTICA_APIKEY carries the API key and
	// CHECKTICA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CHECKTICA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CHECKTICA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCheckticaSDK(opts)
