...
boru-driven command-line client **and** interactive REPL for the Checktica
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/checktica-cli)
make build
...
CHECKTICA_BASE=https://api.example.com ./checktica-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/checktica-cli
   ```
...
Run with no arguments to open a REPL (prompt `checktica>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `detect`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/checktica-cli`. |
...
(`./checktica-cli <expr>`) and interactively (the REPL), and expressions compose the
