...
MCP server exposing the Checktica SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `checktica_list` | `entity`, optional `query` map | First page of records as JSON |
| `checktica_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `checktica_list` and
`checktica_load` tools become available in new sessions.
