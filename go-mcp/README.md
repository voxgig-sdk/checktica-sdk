...
[MCP](https://modelcontextprotocol.io) server exposing the Checktica SDK as
two agent tools — `checktica_list` and `checktica_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/checktica-mcp)
make build

# 2. Provide credentials via the environment
export CHECKTICA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user checktica \
  -- /absolute/path/to/checktica-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./checktica-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// checktica_list: first page of records
{ "entity": "detect" }
{ "entity": "detect", "query": { } }

// checktica_load: one record by id
{ "entity": "detect", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/checktica-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user checktica \
     -- "$PWD"/dist/*/checktica-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `checktica_list` and `checktica_load` tools now appear
   in new sessions. Ask the agent to *"list detect using checktica"*
   and it calls `checktica_list` with `{"entity":"detect"}`.
...
```sh
export CHECKTICA_APIKEY=sk_live_xxx            # API key
export CHECKTICA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `checktica_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `checktica_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CHECKTICA_APIKEY` | API key sent with every request. |
| `CHECKTICA_BASE` | Optional override of the API base URL. |
...
```sh
./checktica-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"checktica_load","arguments":{"entity":"detect","query":{"id":1}}}}'
```
