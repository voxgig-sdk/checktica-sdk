...
class DetectEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DetectEntity($this->_client, $opts);
    }

    /**
     * @param Detect|array $args Detect data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CheckticaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Detect|array The current Detect data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Detect fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CheckticaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Detect fields).
     */
     ...
     * Create a new Detect.
     *
     * @param DetectCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed DetectCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Detect|array The created Detect as an assoc-array at the
     *   SDK boundary; throws CheckticaError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = CheckticaHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
