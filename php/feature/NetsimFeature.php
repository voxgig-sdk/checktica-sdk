...
class CheckticaNetsimFeature extends CheckticaBaseFeature
{
    ...
    private function _simulate(CheckticaContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(CheckticaContext $ctx, array $applied): void
    {
        $client = $this->client;
