...
class CheckticaSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CheckticaHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CheckticaHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CheckticaFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CheckticaFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CheckticaUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CheckticaHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CheckticaHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CheckticaHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CheckticaError($op . "_allow",
                "CheckticaSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CheckticaHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CheckticaError("graphql_error",
                "CheckticaSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Detect($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_detect === null) {
                $this->_detect = new DetectEntity($this, null);
            }
            return $this->_detect;
        }
        return new DetectEntity($this, $data);
    }
