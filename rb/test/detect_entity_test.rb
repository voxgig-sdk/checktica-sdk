...
class DetectEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = detect_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "detect." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CHECKTICA_TEST_DETECT_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    detect_ref01_ent = client.Detect(nil)
...
def detect_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "detect", "DetectTestData.json")
  ...
  entid_env_raw = ENV["CHECKTICA_TEST_DETECT_ENTID"]
  ...
  if env["CHECKTICA_TEST_LIVE"] == "TRUE"
    ...
    client = CheckticaSDK.new(Helpers.to_map(merged_opts))
  end
