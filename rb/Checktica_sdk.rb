...
require_relative 'Checktica_types'
...
class CheckticaSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CheckticaHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CheckticaHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CheckticaFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CheckticaUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CheckticaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CheckticaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CheckticaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = CheckticaHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CheckticaError.new(
        "graphql_error", "CheckticaSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
