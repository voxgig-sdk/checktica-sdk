...
Checktica API detects if a text was written by AI or a human with over 99% accuracy.

Learn more about Checktica API at [checktica.com](https://checktica.com).
...
This SDK exposes the API as a small set of **semantic entities** — Detect — that you
...
```go
client := sdk.Test()
result, err := client.Detect(nil).Create(
...
| --- | --- | --- |
| **Detect** | The Detect entity (create). | `/detect` |
