...
module CheckticaUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CheckticaResponse.new({})
    ...
    if fetch_err
      ...
      response = CheckticaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CheckticaResponse.new(fetched)
    else
