...
class FtCtrl extends CheckticaControl
{
...
class FtClient
{
    ...
    private CheckticaUtility $utility;

    public function __construct(CheckticaUtility $utility, array $options)
    {
    ...
    public function get_utility(): CheckticaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CheckticaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CheckticaContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CheckticaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CheckticaConfig::make_feature($name);
    ...
    private static function buildUrl(CheckticaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CheckticaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CheckticaResponse($fetched);
        }
    ...
    private function fail(CheckticaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CheckticaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
