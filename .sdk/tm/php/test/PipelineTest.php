...
{
    private static function utility(): CheckticaUtility
    {
        return new CheckticaUtility();
    }
    ...
    private static function ctx(array $over = []): CheckticaContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CheckticaContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CheckticaOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CheckticaError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CheckticaOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CheckticaMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CheckticaOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CheckticaMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CheckticaOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CheckticaMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CheckticaSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CheckticaMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CheckticaResponse([]),
            'result' => new CheckticaResult([]),
        ]);
        [, $err] = CheckticaMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CheckticaSpec([]),
            'response' => null,
            'result' => new CheckticaResult([]),
        ]);
        [, $err] = CheckticaMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CheckticaSpec(['step' => 's']),
            'response' => new CheckticaResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CheckticaResult([]),
        ]);
        [, $err] = CheckticaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CheckticaSpec(['step' => 's']),
            'response' => new CheckticaResponse(self::resp(200, ['v' => 1])),
            'result' => new CheckticaResult([]),
        ]);
        [, $err] = CheckticaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CheckticaSpec(['step' => 's']),
            'response' => new CheckticaResponse(self::resp(200, ['v' => 2])),
            'result' => new CheckticaResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CheckticaMakeResponse::call($ctx);
    ...
    {
        $preset = new CheckticaResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CheckticaSpec([]),
            'response' => new CheckticaResponse([]),
            'result' => new CheckticaResult([]),
        ...
        [$got, $err] = CheckticaMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CheckticaResult([])]);
        [, $err] = CheckticaMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CheckticaSpec([]), 'result' => null]);
        [, $err] = CheckticaMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CheckticaSpec(['step' => 's']),
            'result' => new CheckticaResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CheckticaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CheckticaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CheckticaSpec(['step' => 's']),
            'result' => new CheckticaResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CheckticaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CheckticaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CheckticaResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CheckticaSpec([]),
            'result' => new CheckticaResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CheckticaMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CheckticaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CheckticaError('boom', 'boom');
        $utility->fetcher = function (CheckticaContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CheckticaSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CheckticaMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CheckticaContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CheckticaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CheckticaResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CheckticaContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CheckticaSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CheckticaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CheckticaResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CheckticaSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CheckticaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CheckticaMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CheckticaMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CheckticaResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CheckticaDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CheckticaResult(['ok' => false])]);
        $this->expectException(CheckticaError::class);
        CheckticaDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CheckticaResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CheckticaMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CheckticaMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CheckticaBaseFeature();
        $b = new CheckticaBaseFeature();
        CheckticaFeatureAdd::call($ctx, $a);
        CheckticaFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CheckticaBaseFeature
    {
        $f = new CheckticaBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CheckticaFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CheckticaContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CheckticaSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CheckticaPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CheckticaResponse(['headers' => null]),
            'result' => new CheckticaResult([]),
        ]);
        CheckticaResultHeaders::call($ctx);
