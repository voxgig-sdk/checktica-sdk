...
local DetectEntity = {}
DetectEntity.__index = DetectEntity


function DetectEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DetectEntity)
  self._name = "detect"
  self._client = client
...
function DetectEntity:get_name()
  return self._name
end


function DetectEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DetectEntity.new(self._client, opts)
end
...
function DetectEntity:mark_deleted()
  self._deleted = true
end


function DetectEntity:deleted()
  return true == self._deleted
end


function DetectEntity:data_set(args)
  if args ~= nil then
...
function DetectEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DetectEntity:match_set(args)
  if args ~= nil then
...
function DetectEntity:match_get()
...
function DetectEntity:stream(action, args, callopts)
...
function DetectEntity:_run_op(ctx, post_done)
  local utility = self._utility
