...
local CheckticaSDK = {}
CheckticaSDK.__index = CheckticaSDK
...
CheckticaSDK._make_feature = _make_feature


function CheckticaSDK.new(options)
  local self = setmetatable({}, CheckticaSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CheckticaSDK:options_map()
...
function CheckticaSDK:get_utility()
  return Utility.copy(self._utility)
end


function CheckticaSDK:get_root_ctx()
  return self._rootctx
end


function CheckticaSDK:prepare(fetchargs)
...
function CheckticaSDK:direct(fetchargs)
...
-- by passing it.
function CheckticaSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CheckticaSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CheckticaSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function CheckticaSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CheckticaSDK.new(sdkopts)
  sdk.mode = "test"
