...
import (
	...
	omni "github.com/voxgig-sdk/checktica-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CheckticaError); is {
				out := map[string]any{
					"name":    "CheckticaError",
...
func omniCtx(arg any, client *sdk.CheckticaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CheckticaSDK); is {
			client = live
