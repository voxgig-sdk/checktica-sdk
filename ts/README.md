# Checktica TypeScript SDK



The TypeScript SDK for the Checktica API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Detect()` — each with a small set of operations (`create`)
...
```ts
try {
  const detect = await client.Detect().create({ text: "example" })
  console.log(detect)
} catch (err) {
...
| `tester(testopts?, sdkopts?)` | `CheckticaSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `CheckticaSDK.test(testopts?, sdkopts?)` | `CheckticaSDK` | Create a test-mode client. |
...
| `client` | `client(): CheckticaSDK` | Return the parent SDK client. |
...
checktica/
├── src/
│   ├── CheckticaSDK.ts        # Main SDK class
...
await detect.create({ text: "example" })
