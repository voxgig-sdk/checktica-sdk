...
//     (\`new CheckticaSDK(...)\` / \`CheckticaSDK.test(...)\` become
//     \`CheckticaSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"detect":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CheckticaSDK(...)\` and any
// \`CheckticaSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CheckticaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CheckticaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
